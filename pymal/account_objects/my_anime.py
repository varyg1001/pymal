...
class MyAnime(metaclass=singleton_factory.SingletonFactory):
    """
    ...
    __TAG_SEPARATOR = ";"
    __MY_MAL_URL = request.urljoin(consts.HOST_NAME, "editlist.php?type=anime&id={0:d}")
    ...
    def my_reload(self):
        ...
        watched_input = content_rows[contents_divs_index].find(
            name="input", attrs={"id": "completedEpsID", "name": "completed_eps"}
        )
    ...
    def increase(self) -> bool:
        ...
        if self.my_completed_episodes >= self.obj.episodes:
            return False
        if self.my_completed_episodes == 0 and self.my_status != 2:
